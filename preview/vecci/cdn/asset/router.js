String.prototype.router = async function(params) {
    var p = this.toString();
    var s = p.split('/');
    var r = s.filter(g => ["", "#"].some(item => !g.includes(item)));
    var href = r.join('/');
    var uri = (href === "" ? "/" : href);
    console.log(3, {href, p, r, s, uri});
    ...
    var go = async function(resolve, reject) {
        ...
        if (route) {
            ...
            if (!pop && !["blob:"].includes(window.location.protocol)) {
                ...
                var goto = window.hub ? '/templates/' + document.head.querySelector('[name="application-shortname"]').content : '';
                const link =  goto + hash + (route.hash.length > 0 ? route.hash.split('#')[1] : route.path) + route.search;
                console.log(58, {goto, hash, link, route});
